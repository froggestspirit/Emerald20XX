...
static void LoadDisplayMonGfx(u16 species, u32 pid)
{
    ...
    if (species != SPECIES_NONE)
    {
        ...
        if(IsShinyOtIdPersonality(otIdShine, sStorage->displayMonPersonality)) SwapPlttRGB(sStorage->displayMonPalBuffer, (sStorage->displayMonPersonality % 12));
...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    else if (mode == MODE_BOX)
    {
        ...
        if (sStorage->displayMonSpecies != SPECIES_NONE)
        {
            ...
            sStorage->displayMonPalette = GetMonSpritePalFromSpeciesAndPersonality(sStorage->displayMonSpecies, otIdShine, sStorage->displayMonPersonality);
